...
static VALUE Window_create_renderer(VALUE self, VALUE index, VALUE flags)
{
  ...
  if (sdl_renderer == NULL) HANDLE_ERROR(-1);
...
static VALUE Surface_s_save_png(VALUE self, VALUE src, VALUE fname)
{
    SDL_Surface* surface = Get_SDL_Surface(src);
    if (!surface) HANDLE_ERROR(-1);
    return INT2NUM(IMG_SavePNG(surface, StringValueCStr(fname)));
}
...
void rubysdl2_init_video(void)
{
    ...
    rb_define_singleton_method(cSurface, "save_png", Surface_s_save_png, 2);
...
void make_renderers(SDL_Renderer* sdl_renderer, VALUE window)
{
  sdl_renderer = SDL_CreateRenderer(Get_SDL_Window(window), -1, 0);
  if (!sdl_renderer) HANDLE_ERROR(-1);
  VALUE renderer = Renderer_new(sdl_renderer, Get_Window(window));
  rb_iv_set(window, "renderer", renderer);
}

/*
 * Save screenshot containin the current window's contents
 *
 * @return [true] or [false]
 */
static VALUE Window_save_screenshot(VALUE self)
{
  SDL_Rect viewport;
  SDL_Renderer* sdl_renderer;
  VALUE renderer = rb_iv_get(self, "renderer");
  if (renderer == Qnil) {
    make_renderers(sdl_renderer, self);
  } else {
    sdl_renderer = Get_SDL_Renderer(renderer);
  }
  SDL_RenderGetViewport(sdl_renderer, &viewport);
  SDL_Surface *surf = SDL_CreateRGBSurface(0, viewport.w, viewport.h, 32, 0, 0, 0, 0);
  if (!surf) {
    elog("Cannot create SDL_Surface: ", 0);
    return Qfalse;
  }
  if ( SDL_RenderReadPixels(sdl_renderer, 0, surf->format->format, surf->pixels, surf->pitch ) != 0) {
    elog("Cannot read data from SDL_Renderer: ", 0);
    return Qfalse;
  }
  VALUE shot = rb_const_get(mSDL2, rb_intern("Screenshot"));
  VALUE directory = rb_iv_get(shot, "directory");
  VALUE format = rb_iv_get(shot, "format");
  make_screenshots_dir(directory);
  char* dir = StringValueCStr(directory);
  char fn[500];
  make_filename(dir, fn, rb_iv_get(shot, "filename"), format);
  if (!rb_str_cmp(format, rb_str_new_cstr("jpg"))) {
    if ( IMG_SaveJPG(surf, fn, 95) != 0 ) {
      elog("Freeing JPG surf after failure", surf);
      return Qfalse;
    }
  }
  if (!rb_str_cmp(format, rb_str_new_cstr("png"))) {
    if ( IMG_SavePNG(surf, fn) != 0 ) {
      elog("Freeing PNG surf after failure", surf);
      return Qfalse;
    }
  }
...
void rubysdl2_init_image(void)
{
  ...
  IMG_format_init(IMG_INIT_JPG);
  IMG_format_init(IMG_INIT_PNG);
  IMG_format_init(IMG_INIT_TIF);
  IMG_format_init(IMG_INIT_WEBP);
  rubysdl2_init_screenshot();
  // Save Screenshot
  rb_define_method(cWindow, "save_screenshot", Window_save_screenshot, 0);
}
